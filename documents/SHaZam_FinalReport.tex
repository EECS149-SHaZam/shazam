...
Originally, our project was designed such that the light would track the user's gaze, down to where he or she was looking at the desk. However, our algorithm proved too unstable for this task, so we reduced the problem to having the light simply follow the user. Moving forward, we would like to explore more robust algorithms to solve the original problem. 

In addition, the original algorithm uses trigonometric calculations as its basis, to find the appropriate pitch and yaw to move the motors. However, we could explore other options of tracking the user's face. For instance, if we used the Wiimote's internal accelerometer, we could get a grasp of what the user's head movements are, to supplement the existing algorithms that we had. Furthermore, we could look into facial tracking or eyetracking, or take it to the next level by investigating computer vision as a way to solve this problem. These solutions would add to the featureset as well as provide more accurate, more stable, and more definitive means towards tracking the user.
